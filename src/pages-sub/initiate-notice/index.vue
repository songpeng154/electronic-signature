...
import { addNotice } from '@/service/api/notice.ts'
...
const notificationType = ref<any[]>([])
...
<template>
    ...
    <div class="flex-1 overflow-hidden p-10px">
      <sar-card root-class="flex-1 h-full overflow-auto">
        <sar-form
          ...
          :rules="rules"
        >
          <sar-form-item label="通知名称" name="name">
            <sar-input
              v-model="form.name"
              clearable
              placeholder="请输入"
          ...
          <sar-form-item label="通知描述" name="descriptions">
            <sar-input
          ...
          <sar-form-item label="通知类型" name="type">
            <sar-picker-input
              v-model="form.type"
              clearable
              placeholder="请选择"
          ...
          <sar-form-item label="完成期限" name="completeTime">
  ...
  <sar-popout v-model:visible="deptVisible" title="标题" @confirm="onDeptConfirm">
